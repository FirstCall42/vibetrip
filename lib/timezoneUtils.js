 ...
 * Handles formatting and local timezone offsets
 * 
 * FLOW:
 * User Input → LOCAL TIME WITH OFFSET → Store in DB
 * DB (Offset/UTC String) → Parse and display in target timezone
 */
...
/**
 * Convert LOCAL time to ISO 8601 string with offset
 * Example: User enters 15:35 in New York timezone (EDT = UTC-4)
 *   → 15:35 EDT becomes 2026-06-16T15:35:00-04:00
 * 
 ...
 */
export function localToTzString(dateStr, timeStr, timezone) {
  if (!dateStr) return '';
  const actualTime = timeStr || '00:00';
  const [year, month, day] = dateStr.split('-').map(Number);
  const [hours, minutes] = actualTime.split(':').map(Number);

  // Create a reference UTC date for this calendar day to find the offset
  ...
  // Format the offset as ISO offset (+HH:MM or -HH:MM)
...
/**
 * Format an ISO string to a clean GMT offset representation (e.g. GMT+1 or GMT-4)
 * @param {string} isoString - Date-time string containing offset (or ending in Z)
 * @returns {string} GMT representation (e.g., 'GMT-4', 'GMT+1', or 'GMT')
 */
export function getGmtOffsetDisplay(isoString) {
  if (!isoString) return '';
  if (isoString.endsWith('Z')) return 'GMT';
  const match = isoString.match(/([+-])(\d{2}):?(\d{2})$/);
  if (match) {
    const [_, sign, hours, mins] = match;
    const hourNum = parseInt(hours, 10);
    const minSuffix = parseInt(mins, 10) > 0 ? `:${mins}` : '';
    return `GMT${sign}${hourNum}${minSuffix}`;
  }
  return '';
}
